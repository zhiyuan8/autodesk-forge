...
router.post('/jobs', async (req, res, next) => {
    ...
    job.input.rootFilename = 'Autodesk_Hospital_HVAC.rvt';
    job.output = new JobPayloadOutput([
        new JobSvfOutputPayload()
    ]);
    job.output.formats[0].type = 'svf';
    job.output.formats[0].views = ['2d', '3d'];
    // job.output.formats[0].advanced = {'generateMasterViews' : true}; ## This is to include rooms only if there exist 'space' in Revit models ##
    try {
